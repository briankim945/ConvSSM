...
def discretize_zoh_n(Lambda, B_tilde, Delta):
    """ Discretize a diagonalized, continuous-time linear SSM
        using zero-order hold method.
        Args:
            Lambda (complex64): diagonal state matrix              (P, H)
            B_tilde (complex64): input matrix                      (P, H)
            Delta (float32): discretization step sizes             (P,)
        Returns:
            discretized Lambda_bar (complex64), B_bar (complex64)  (P, H), (P,H)
    """
    ###### DOUBLE-CHECK THE MATH ON THIS
    Identity = np.ones(Lambda.shape)
    Lambda_bar = np.exp(Lambda * np.repeat(np.expand_dims(Delta, 1), Lambda.shape[-1], axis=-1))
...
class ConvS5SSM_FFT(nn.Module):
    ...
    def setup(self):
        ...
        # Initialize state to state (A) kernel (assuming each state step is same shape)
        self.A = self.param("A",
                            lambda rng, shape: init_CV(rng, shape, self.V),
                            (self.U, 2*self.P, self.k_A))
        A_tilde = self.A[..., 0] + 1j * self.A[..., 1]
        # print("self.A:", self.A.shape)
        # print("A_tilde:", A_tilde.shape)
        self.A_bar = A_tilde.reshape(self.P, self.U, self.k_A, self.k_A).transpose(2, 3, 1, 0)
        ...
        self.B_bar = B_tilde.reshape(self.P, self.U, self.k_B, self.k_B).transpose(2, 3, 1, 0)
        ...
        #     # Discretize
        #     # Handling 2-D A
        #     A_tilde = A_tilde.transpose(2, 3, 0, 1).reshape(self.P, self.U * self.k_A * self.k_A)
        #     self.A_bar, self.B_bar = discretize_zoh_n(A_tilde,
        #                                             B_tilde,
        #                                             step)
        #     self.A_bar = self.A_bar.reshape(self.P, self.U, self.k_A, self.k_A).transpose(2, 3, 1, 0)
        ...
        #     def init_discrete():
        #         A_tilde = A_tilde.transpose(2, 3, 0, 1).reshape(self.P, self.U * self.k_A * self.k_A)
        #         A_bar, B_bar = discretize_zoh_n(A_tilde,
        #                                       B_tilde,
        #                                       step)
        #         A_bar = A_bar.reshape(self.P, self.U, self.k_A, self.k_A).transpose(2, 3, 1, 0)
    ...
    def __call__(self, input_sequence, x0):
        ...
        if self.parallel:
            ...
            x_last, ys = diagonal_scans.apply_convSSM_sequential(self.A_bar,
                                                                 self.B_bar,
                                                                 self.C_tilde,
                                                                 input_sequence,
                                                                 x0)
...
def init_ConvS5SSM_FFT(ssm_size,
                   blocks,
                   clip_eigs,
                   U,
                   k_A,
